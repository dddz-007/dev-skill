---
name: dev-skill
description: 通用的全栈开发协作工作流，适用于新功能、已有功能修改、单独前端/后端/数据库任务、问题修复、代码审查、联调、测试和发布前检查。要求 Agent 先调查和制定方案，优先复用已有实现，严格控制修改范围，对数据库和其他高风险操作设置确认门禁，并用证据交付结果。
metadata:
...
低能力模型、上下文较短或用户要求“按步骤执行”时，必须：一次只做一个阶段；编辑前列出文件和意图；数据库变更前列出环境和脚本摘要；用 `[ ]` 待处理、`[x]` 有证据通过、`[!]` 被阻塞记录进度；缺少文件、权限、工具或信息时停止并说明；失败后保留错误、做一次针对性修复并重跑；绝不把沉默当确认。

## 不可违反的安全规则

1. 严禁暴露、打印、提交或硬编码密码、令牌、Cookie、私钥和其他机密。凭据只通过安全方式使用，不写入代码、日志、Skill、规则文件或版本库。
2. 数据库未明确环境和授权时，默认只读；无法证明不是生产库时，按生产库处理，禁止变更。
3. 生产库操作、删除表/字段/数据、批量更新、修改主键或唯一约束、不可逆脚本、破坏性文件操作、登录/权限/支付/订单逻辑变更、破坏兼容的接口变更、核心依赖升级，必须先用中文风险提示说明并等待用户对具体操作和具体环境明确确认。
...
按可用程度执行：Diff 检查；格式化、静态检查、类型检查；后端编译和测试；前端构建和测试；数据库迁移演练/结构验证；接口调用；端到端正常流程；错误参数、空数据、权限、重复提交、边界和回归检查；UI 实际页面和交互检查。
